...
import (
	...
	"github.com/nikmy/algo/tools/slices"
)
...
func (m *Matrix[T]) Add(rhs *Matrix[T]) error {
	if m.Shape() != rhs.Shape() {
		return ErrSizeMismatch{shapes: [2][2]int{m.Shape(), rhs.Shape()}}
	}
...
func (m *Matrix[T]) Multiplied(rhs *Matrix[T]) (*Matrix[T], error) {
	if m.Cols() != rhs.Rows() {
		return nil, ErrSizeMismatch{shapes: [2][2]int{m.Shape(), rhs.Shape()}}
	}
...
func (m *Matrix[T]) Pow(exp int) error {
	if m.Rows() != m.Cols() {
		return ErrSizeMismatch{shapes: [2][2]int{m.Shape(), m.Shape()}}
	}
